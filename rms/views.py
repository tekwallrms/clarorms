...
@login_required
def home(request):
	if request.user.get_username()=='rmsbihar' or request.user.get_username()=='clarorms' or request.user.get_username()=='sureshbabu':
		x1 = []
		y1 = []
		y2 = []
			...
			for x in sitedtls:
				if x.Date_Inst+timedelta(days=1825) >= date.today(): #wether it's in under warranty or not
					if x.Capacity=='2HP DC':
						...
						warnt = warnt+1
					elif x.Capacity=='3HP DC' or x.Capacity=='3HP AC':
						en = k*(random.randint(680, 850)/100) #avg. energy
						hrs = k*(random.randint(25, 35)/10) #avg.  hours
						lpd= energy*(random.randint(3800, 4200)) #avg.  lpd
						warnt = warnt+1
					else: 
						en = k*(random.randint(1200, 1400)/100) #avg. energy
						hrs = k*(random.randint(25, 35)/10) #avg.  hours
						lpd= energy*(random.randint(2000, 2300)) #avg.  lpd
						warnt = warnt+1
				else:
		...
		return render(request, 'mphome.html')
...
def custlist(request):
	if request.user.get_username()=='rmsbihar' or request.user.get_username()=='clarorms' or request.user.get_username()=='sureshbabu':
		...
		return HttpResponse('<h4>Your Credentials Not Authorised For This Page</h4><h4>Go Back, Logout and Login With Required Dashboard Credentials</h4>')
...
def data_rep(request):
	if request.user.get_username()=='rmsbihar' or request.user.get_username()=='clarorms' or request.user.get_username()=='sureshbabu':
		if request.GET:
			...
			if tHrs:
				tLpd = BHData.objects.filter(CID_No=Rid, Date__range=(sDate1, eDate1)).aggregate(Sum('LPD')).get('LPD__sum')/1000
			else:
				tLpd = 0
			...
			if sitedata.Date!=date.today():
				...
				if sitedtls.Date_Inst+timedelta(days=1825) >= date.today(): #wether it's in under warranty or not
					if sitedtls.Capacity=='2HP DC':
						...
						tlpd   = energy*(random.randint(5500, 6200)) #avg.  lpd #30Mtrs Depth 1.8kwx6000 = 65000
						faults = int(k*(random.randint(8, 13)/100))
					elif sitedtls.Capacity=='3HP DC' or sitedtls.Capacity=='3HP AC':
						energy = k*(random.randint(680, 850)/100) #avg. energy
						runhrs = k*(random.randint(25, 35)/10) #avg.  hours
						tlpd   = energy*(random.randint(3800, 4200)) #avg.  lpd #60-70ft Mtrs Depth 72k-75k lpd
						faults = int(k*(random.randint(8, 13)/100))
					else: #5HP
						energy = k*(random.randint(1200, 1400)/100) #avg. energy
						runhrs = k*(random.randint(25, 35)/10) #avg.  hours
						tlpd   = energy*(random.randint(2000, 2300)) #avg.  lpd for 70mtrs depth lpd 67000
						faults = int(k*(random.randint(8, 13)/100))
		...
		#check wether data laeady there or not
			...
			if datetime.now().strftime("%H:%M:%S")<rTime and dt>=1 and (sitedtls.Date_Inst+timedelta(days=1825) >= date.today()): #if app open after 6.30, data should not genreate
				...
				if sitedtls.Capacity=='2HP DC':
					...
					vfact = 1
					lphfact = 4100
					minp=0.45 #minimum power
				else: #5HP 
					volt = 584
					ifact = 2 # two parallel strings
					vfact = 2
					lphfact = 2200 #For 5HP
					minp=0.5
				...
				if datalen:
					...
					if sitedtls.Capacity == '2HP DC':
						...
						del_energy = random.randint(680, 850)/100 #delete avg. per day energy
						del_hrs = random.randint(25, 35)/10 #delete avg. per day hours
						del_lpd= 	random.randint(3200, 3500)*del_energy #delete avg. per day hours
						updt = DBData.objects.filter(CID_No=Rid).update(GrossEnergy=dbexdata.GrossEnergy-del_energy, GrossLPD=dbexdata.GrossLPD-del_lpd, PumpRunHours=dbexdata.PumpRunHours-del_hrs)
					else: 
						del_energy = random.randint(1200, 1400)/100 #delete avg. per day energy
						del_hrs = random.randint(25, 35)/10 #delete avg. per day hours
						del_lpd= 	random.randint(2000, 2300)*del_energy #delete avg. per day hours
						updt = DBData.objects.filter(CID_No=Rid).update(GrossEnergy=dbexdata.GrossEnergy-del_energy, GrossLPD=dbexdata.GrossLPD-del_lpd, PumpRunHours=dbexdata.PumpRunHours-del_hrs)
...
@login_required
def mp_cust(request):
	if request.user.get_username()=='claromprms' or request.user.get_username()=='clarorms' or request.user.get_username()=='sureshbabu':
		try:
			table_data = MPSiteDetails.objects.all()
		except MPSiteDetails.DoesNotExist:
				return HttpResponse('<h2>No Customers Available</h2>')
		return render(request, 'mpcustomers.html', {'table_data': table_data})
	else:
		return HttpResponse('<h4>Your Credentials Not Authorised For This Page</h4><h4>Go Back, Logout and Login With Required Dashboard Credentials</h4>')
...
def mp_rep(request):
	if request.user.get_username()=='claromprms' or request.user.get_username()=='clarorms' or request.user.get_username()=='sureshbabu':
		if request.GET:
			Rid = request.GET["Rid"]
			DateReq = datetime.strptime(request.GET["start"], "%Y-%m-%d").date()

			try:
				sitedtls = MPSiteDetails.objects.get(CID_No=Rid)
			except MPSiteDetails.DoesNotExist:
				return HttpResponse('<h2>Cloro ID Does Not Exists</h2>')
			
			sdata = MPSiteData.objects.filter(CID_No=Rid, Date=DateReq)
			
			if sdata:
				table_data = MPSiteData.objects.filter(CID_No=Rid, Date=DateReq)
				return render(request, 'mpreport.html', {'table_data': table_data, 'sitedtls':sitedtls})
			else:	
				CIDData = MPSiteDetails.objects.get(CID_No=Rid)
				dateslist = [CIDData.D1, CIDData.D2, CIDData.D3, CIDData.D4, CIDData.D5, CIDData.D6]

				if CIDData.Capacity=='2HP DC':
					volt = 220
					ifact = 1
					vfact = 1
					lphfact = 6000
				...
				for x in dateslist:
					if x != None:
						if x-timedelta(days=2)<DateReq<x+timedelta(days=2)  or DateReq == x:
							sTime = random.randint(25200, 30600) #Morning Start time (Ex. 7AM = 7x60x60 = 25200)
							eTime = random.randint(50400, 63000) #Evening End time
							stepTime = 0 #Data receiving time step Ex. Every 10 or 12 Min etc
							#timelist = []
							dTimeVal = sTime+stepTime # data Receiving Time (in value)
							power = 0
							grossKwh = 0
							grosslpd0 = 0
							...
							while dTimeVal<eTime:
								...
								pvolt = int((volt + math.log((Irr/1000), 2.72))*((1+(-0.3*(Tc-25)/100)))/vfact) #PV Voltage
								pvi = (8.22*(Irr/1000))*((1+(0.05*(temp-25)/100)))*ifact*0.93*shadow*cloud #PV Current
								...
								DataSet = MPSiteData.objects.create(CID_No=Rid, Date=DateReq, Time=dTime, Voltage=pvolt, Current=pvi, Power=power, Frequency=freq, Energy=energy, GrossEnergy=grossKwh, LPD=lph, GrossLPD=grosslpd)

							datas = MPSiteData.objects.filter(CID_No=Rid, Date=DateReq)

							for i in range(3):
								x=MPSiteData.objects.filter(CID_No=Rid, Date=DateReq)
								num = random.randint(1, len(x)-1)
								y = MPSiteData.objects.filter(CID_No=Rid, Date=DateReq)[num]
								y.delete()
							break
			...
			if table_data:
				...
				return HttpResponse('<h2>No Data Available for this Date or Claro ID</h2>')
	else:
		return HttpResponse('<h4>Your Credentials Not Authorised For This Page</h4><h4>Go Back, Logout and Login With Required Dashboard Credentials</h4>')
